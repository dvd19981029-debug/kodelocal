...
interface EditarFraganciaModalProps {
  perfume: CatalogoItem | null;
  onClose: () => void;
  onSuccess: () => void;
  showToast: (msg: string, type?: 'success' | 'error' | 'info') => void;
}

export function EditarFraganciaModal({
  perfume,
  onClose,
  onSuccess,
  showToast,
}: EditarFraganciaModalProps) {
  ...
  useEffect(() => {
    if (perfume) {
      setForm({
        ...
        marca_inspirada: perfume.marca_inspirada || '',
        genero: perfume.genero || 'Caballero',
        precio_normal: parseFloat(perfume.precio_normal?.toString() || '20'),
        precio_extra_shot: parseFloat(perfume.precio_extra_shot?.toString() || '25'),
        activo: perfume.activo !== false,
      });
    }
  }, [perfume]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        showToast('Fragancia actualizada correctamente', 'success');
        onSuccess();
        onClose();
      } else {
        showToast(data.error || 'Error al actualizar fragancia', 'error');
      }
    } catch (err: any) {
    ...
    <div className="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto">
      <div className="bg-white rounded-2xl max-w-lg w-full p-6 space-y-4 shadow-2xl border border-slate-200 animate-in fade-in zoom-in-95 my-8">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-sm font-extrabold text-slate-900">
                Editar Fragancia #{perfume.codigo}
              </h3>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 text-xs">
          {/* Disponibilidad / Estado Activa vs Inactiva */}
          <div
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <span className="font-extrabold block text-xs">
                  {form.activo ? 'FRAGANCIA ACTIVA (Disponible)' : 'FRAGANCIA INACTIVA (Agotada)'}
                </span>
